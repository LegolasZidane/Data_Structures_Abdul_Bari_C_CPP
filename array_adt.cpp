/*Before diving into this code, i want you to help you with you overall experience.
...
void insert(struct Array *arr, int x) //When you want to insert an element in a sorted list. Index is not needed here.
{
    if(arr->length == arr->size)
        return;
    else
...
int linearSearchTransposition(struct Array *arr, int x) // Original array mei element will keep on coming close to the start index on the basis of its requirement.
{
    for(int i = 0; i<(*arr).length; i++)
    {
        if(x == (*arr).A[i])
        {
            swp(&arr->A[i], &arr->A[i-1]);//Debug karna zaruri with or without & works the same. Interesting!
            return i-1;
        }
    }
    return -1;
...
int linearSearchTranspositionEnhanced(struct Array *arr, int x) // Original array mei seached element will come to the beginning for optimized search next time. 
{
    for(int i = 0; i<(*arr).length; i++)
    {
        if(x == (*arr).A[i])
...
int binarySearch(struct Array arr, int key)//Better search algorithm than linear but requires the list to be sorted
{
    int low = 0;
...
void reverseEnhanced(struct Array *arr)//Two pointers concept
{
    int i,j;
...
void leftShift(struct Array *arr)//Shift all elements of the list by one(left). Right most element becomes 0 and the first element is lost.
{
    int i;
...
void leftRotate(struct Array *arr)//Shift all elements of the list by one(left). Right most element becomes the first element.
{
    int i, temp = arr->A[0];
    for(i = 0; i<arr->length-1; i++)
        arr->A[i] = arr->A[i+1];
...
void rightShift(struct Array *arr)//Shift all elements of the list by one(right). Left most element becomes 0 and the last element is lost.
{
    int i;
...
void rightRotate(struct Array *arr)//Shift all elements of the list by one(right). Left most element becomes last element.
{
...
void rearrange(struct Array *arr)//-ve values on the left side and +ve on the right side
{
...
void merge(struct Array *a, struct Array a1, struct Array a2)//arrays must be sorted, remember!
{
    int i=0, j=0, k=0;
...
void intersectionSorted(struct Array *a, struct Array a1, struct Array a2)//arrays must be sorted, remember!
{
    int i=0, j=0, k=0;
    while(i<a1.length && j<a2.length)
    {
...
void differenceSorted(struct Array *a, struct Array a1, struct Array a2)//a1 - a2
{
    ...
    {
        if(a1.A[i]<a2.A[j])
            a->A[k++] = a1.A[i++]; // being the smaller number is the only guarantee that the element won't be in a2.
        else if(a1.A[i]>a2.A[j])  
            j++; // if element in a1 is larger than current element in a2, there might be a chance that this element in a1 could be in a2, think about it.
        else
        {
            i++;
            j++;
